...
すなわち,**引数がlua_Stateポインタであり,戻り値がintである関数のみをスタックに積むことができます.**ただし,**この関数の戻り値には,Luaで使用される関数の戻り値の数を指定します.**例えば,`script4-1.lua`で定義された`arithmetic`関数をC++側で定義すると,今回のサンプルソースコードの様になります.
...
コルーチンの役割を果たすco関数の中に,`coroutine.yield`がいくつかあります.この`coroutine.yield`の部分で処理が中断されます.このとき,`coroutine.yield`の引数に何かしらの値を指定すると,引数に指定した値をコルーチン関数の戻り値とすることができます.もちろん,何も指定しなくてもおkです.
...
[次回の13日目の記事](../13_0)では,実際にLuaスクリプトをAltseed製のゲームに組み込むサンプルを示していきます.どうぞお楽しみください.
